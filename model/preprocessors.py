...
def process_targets(perf_series: pd.Series, target_date:int) -> pd.Series:
    """
    Creates targets consisting of data {target_date} days after the reference date (i.e. the most recent input's date)
    :param perf_series: The stock price returns data to extract targets from.
    :return: A series where the values consist of returns {target_date} after the reference dates, and where the index consists
    of the reference dates.
    """
    return perf_series.shift(-target_date).dropna()
